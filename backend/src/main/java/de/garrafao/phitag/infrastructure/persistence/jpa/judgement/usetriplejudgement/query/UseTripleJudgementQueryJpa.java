...
import de.garrafao.phitag.domain.judgement.usetriplejudgement.query.*;
import de.garrafao.phitag.infrastructure.persistence.jpa.core.SpecificationCombiner;

public class UseTripleJudgementQueryJpa implements Specification<UseTripleJudgement> {

    private final Query query;

    public UseTripleJudgementQueryJpa(final Query query) {
        this.query = query;
    }

    @Override
    public Predicate toPredicate(Root<UseTripleJudgement> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        final List<Specification<UseTripleJudgement>> specifications = new ArrayList<>();
