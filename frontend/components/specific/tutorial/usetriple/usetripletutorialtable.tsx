...
import UseTripleInstance from "../../../../lib/model/instance/usetripleinstance/model/UseTripleInstance";
import AddUseTripleJudgementCommand from "../../../../lib/model/judgement/usetriplejudgement/command/AddUseTripleJudgementCommand";


interface IResult {
    data: AddUseTripleJudgementCommand[];
    instances: UseTripleInstance[];
}

// TODO: refactor when project
const UseTripleTutorialTable: React.FC<IResult> = ({ data, instances }) => {
    return (
                        ...
                        {data.map((judgement, i) => {
                            return (
                                <tr key={i}>
                                    ...
                                    <td className="px-6 py-4 overflow-auto font-dm-mono-light">
                                        <span key={i} className="tooltip group w-fit">
                                            {getFormatedShortUsage(instances[i].getThirdusage())}
                                            <div className="tooltip-container group-hover:scale-100">
                                                <div className="whitespace-nowrap mx-4 my-2">
                                                    Data ID: {instances[i].getThirdusage().getId().getDataid()}
                                                </div>
                                            </div>
                                        </span>
                                    </td>
...
export default UseTripleTutorialTable;
