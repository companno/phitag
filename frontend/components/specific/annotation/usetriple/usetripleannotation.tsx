...
import { annotateUsetriple } from "../../../../lib/service/judgement/JudgementResource";
import LoadingComponent from "../../../generic/loadingcomponent";
import Usage from "../../../../lib/model/phitagdata/usage/model/Usage";


const UseTripleAnnotation: React.FC<{ phase: Phase }> = ({ phase }) => {


    // States
    const [annotation, setAnnotation] = useState({
        instance: null as unknown as UseTripleInstance,
        comment: "",
    ...
    const handleSubmitAnnotation = (judgement: string) => {
        ...
        if (resultCommand !== null) {
            annotateUsetriple(resultCommand, storage.get)
                .then((result) => {
                    fetchNewAnnotation();
                }).catch((error) => {
    ...
    const fetchNewAnnotation = () => {
        fetchRandomInstance<UseTripleInstance, UseTripleInstanceConstructor>(phase.getId().getOwner(), phase.getId().getProject(), phase.getId().getPhase(), (new UseTripleInstanceConstructor()), storage.get)
            .then((instance) => {
                setAnnotation({
                    ...annotation,
          ...
          <div className="w-full flex flex-row justify-center space-y-4 ">
              ...
              <div>
                 <thead className="font-bold text-lg">
                     <tr>
                        <th scope="col"
                             className="ThirdLemma">
                             Category: <div>{annotation.instance.getLemma3()}</div>
                         </th>
                         <th scope="col"
                             className="ThirdGroup">
                             Character: <div>{annotation.instance.getGroup3()}</div>
                         </th>
                     </tr>
                 </thead> 
                 <UsageField key={2} usage={annotation.instance.getThirdusage()} />
              </div>
          </div>
...
export default UseTripleAnnotation;

function verifyResultCommand(phase: Phase, judgement: string, annotation: {
    instance: UseTripleInstance;

    comment: string;
    initialLoad: boolean;
}): AddUseTripleJudgementCommand | null {
    ...
    return new AddUseTripleJudgementCommand(
